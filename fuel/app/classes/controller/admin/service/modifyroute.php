...
class Controller_Admin_Service_Modifyroute extends Controller_Admin_App
{
			...
			if($route_check) {
				...
				if(isset($_POST['page'])) {
					...
					if($_POST['page'] == 'modify_route') {
						...
						$data['main_image_url'] = isset($_POST['main_image_url']) ? trim($_POST['main_image_url']) : '';
						...
						if(isset($_POST['route_name']) && isset($_POST['route_description']) && isset($_POST['route_price_min']) && isset($_POST['route_price_max']) 
							...
							//输入内容检查
							$result_check = Model_Route::CheckUpdateRoute($param_update);
					...
					$data['input_route_name'] = $route_info['route_name'];
					$data['input_route_description'] = $route_info['route_description'];
					$data['input_route_price_min'] = $route_info['route_price_min'];
					$data['input_route_price_max'] = $route_info['route_price_max'];
					$data['input_route_base_cost'] = $route_info['route_base_cost'];
					$data['input_route_traffic_cost'] = $route_info['route_traffic_cost'];
					$data['input_route_parking_cost'] = $route_info['route_parking_cost'];
					$data['input_route_status'] = $route_info['route_status'];
					//主图
					...
					//景点列表
					foreach($route_info['detail_list'] as $detail_info) {
						$route_spot_list_tmp = $detail_info['route_spot_list'];
						$route_spot_list = array();
						foreach($route_spot_list_tmp as $spot) {
							$route_spot_list[] = $spot['spot_id'];
						}
						...
						$data['input_detail_list'][] = array(
							...
							'route_spot_list' => $route_spot_list,
				...
				//调用View
				return Response::forge(View::forge($this->template . '/admin/service/modify_route', $data, false));
			} else {
				return Response::forge(View::forge($this->template . '/admin/error/access_error', $data, false));
			}
//		} else {
